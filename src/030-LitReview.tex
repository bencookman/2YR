...
\end{equation}
($\und{V}$ is the vector of primitive variables, $\undt{\vb{A}}$ represents the convection of these variables and $\und{b}$ are their source terms) and solving the left-eigenvalue problem of $\undt{\vb{A}}$ to find its left eigenvectors $\und{l}_m^T$. They then prove that enforcing the condition of $\und{l}_m^T \partial \und{V} / \partial t = 0$ gives precisely no wave entering the boundary in this continuous problem.
