...
Later on, a \emph{recipe} to create characteristic BCs would be given in \cite{thompson1987LecturesSeriesComputational, thompson1987TimeDependentBoundary, thompson1990TimeDependentBoundaryConditions} which is the basis not just for many modern day non-reflecting boundary formulations, but may also be used for wall boundaries and several over sub- and supersonic inflow and outflow boundary conditions. This method is referred to as the locally one-dimensional inviscid (LODI) approximation, since all diffusive and transverse terms are ignored. The approximation is explained in detail in \sect{sec:LODI}, but briefly: the same characteristic analysis is performed as above, except the hyperbolic equations are projected onto each `basis' eigenvector:
\begin{equation}
\pdv{\und{V}}{t} + \undt{\vb{A}} \cdot \vb{\nabla} \und{V} + \und{b} = 0
\quad \iff \quad
\und{l}_m^T \pdv{\und{V}}{t} + \cl{L}_M + \und{l}_m^T \und{b} = 0
\end{equation}
where $\und{b}$ holds source terms (e.g. gravity, chemical production rate) and $\cl{L}_M \equiv λ_m \und{l}_m^T \vb{\nabla} \und{V}$. The eigenvalue $λ_m$ represents the speed of that characteristic. What's left, then, is to evaluate 
