...
% As mentioned above, the process of meshing a domain is often expensive. This is exacerbated when high-order methods are used since mesh elements are forced to be larger.
% Mesh-free methods circumvent this issue by proposing discretisations without connectivity, relying only on either local data or all data.
% Despite this, they are far less developed / well used than mesh based methods, although many mesh based methods are commonly used for fluid and combustion problems
% \cite{li2002MeshfreeParticleMethods}, \cite{garg2018MeshfreeMethodsComprehensive}

% RBFs are a well-explored method and are based off the idea that a field known at discretely many points in a domain may be interpolated at arbitrary locations, provided we accurately decompose the field onto a space of a radial basis function centred at the different discretisation nodes.
% This decomposition requires we solve a system of linear equations at each point we interpolate to as a preprocessing step for each location
% This is extended to PDEs by simply differentiating our interpolated field and in so doing differentiating the RBF at each ...
% This is the Kansa method propsed in....

% Although they are rarely employed for combustion problems, \cite{kansa2009NumericalSimulationTwodimensional} investigates their use to model the flame via a simple G-equation.
% Applicability to combustion is constrained because of the global approach in this formalism, which greatly increases computational expense, a significant bottleneck for combustion simulation.

% Compact RBFs offer a solution to this -- these instead require only a local stencil containing only the nodes within a circle of size $h$ surrounding the point. By minimising this stencil size so only a minimum number of nodes are contained within such that order conditions can be satisfied, the rate at which the linear system can be solved and the resulting weights applied is minimised \cite{buhmann2003RadialBasisFunctions}.

% 

% Despite this, it is the knowledge of the author yet to be applied to combustion simulation

% Another compact method are that of GFDM \cite{jensen1972FiniteDifferenceTechniques}, which similarly solves a linear system for the gradient operator at each node as a preprocessing step (assuming a Lagrangian formulation is not used), but instead solves for order conditions directly by solving for each term of the taylor series approximation of the gradient operators up to a desired order using the minimal number of nodes.
...
% LABFM is to GFDM what RBF-FD is to compact RBFs: by increasing the support radius, computational cost increases, but stability and accuracy properties increase greatly
% And provide improved applicability to multiple kernels [How to cite Henry's unpublished work]
