...
\subsection{Characteristic Boundary Conditions for Reacting Flows}

\cite{thompson1987TimeDependentBoundary, thompson1990TimeDependentBoundaryConditions, poinsot1992BoundaryConditionsDirect, poinsot2001TheoreticalNumericalCombustion, sutherland2003ImprovedBoundaryConditions}
...
% A condition for a Non-reflecting boundaries is first given in \cite{hedstrom1979NonreflectingBoundaryConditions} for the inviscid gasdynamics equations.
% It involves performing characteristic analysis by writing the hyperbolic conservation equations in their primitive form $ $ and solving the left-eigenvalue problem on $A$ to find left eigenvectors $\und{l}_m^T$
% They prove that enforcing the condition $\und{l}_m^T \partial \und{V} / \partial t = 0$ gives precisely no waves entering the boundary in the continuous problem
% This equation is the basis of all characteristic boundary conditions
