...
\end{equation}
where $c_i$ is the local sound speed given by $c_i^2 = p_i / \r_i$ and $\cl{D}_{\rm{mom}, i}$, $\cl{D}_{\a, i}$ and $\cl{D}_{\rm{th}, i}$ are are the diffusion constants defined locally as:
\begin{equation}
\cl{D}_{\rm{mom}, i} = \mu_i / \r_i, \quad \cl{D}_{\a, i} = D_{\a, i}, \quad \cl{D}_{\rm{th}, i} = \l_i / \r_i c_{p, i}.
\end{equation}
The constants $C_\rm{aco}$, $C_\rm{mom}$, $C_\rm{sp}$ and $C_\rm{th}$ are determined separately by the stability of time integrator.
