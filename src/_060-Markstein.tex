...
\begin{subequations}
\begin{boxaliat}{2}
\label{eqn:hydro-incomp} \vnab \cdot\vb{u} &= 0                         &&+ \cl{O}\left( δ^2 \right), \\
\label{eqn:hydro-u} ρ \mdv{\vb{u}}{t} &= - \vnab p + δ\Pr \lap\vb{u} &&+ \cl{O}\left( δ^2 \right).
\end{boxaliat}
\end{subequations}
...
\end{equation}
Hence, considering the advection equation for the flame: $\partial c / \partial t + \ndt{\vb{f}}\cdot \vnab c = 0$ we find the following simple expressions for absolute and displacement speed:
\begin{align}
S_a &= \frac{1}{\norm{\vnab c}} \pdv{c}{t} \,\bigg|_{F^-} \\
S_d &= \frac{1}{\norm{\vnab c}} \mdv{c}{t} \,\bigg|_{F^-}
\end{align}
In this context, the species conservation equation may be used:
\begin{align}
ρ \mdv{c}{t} &= \frac{1}{\bbra{Y_α}_V} \cdot ρ \mdv{Y_α}{t} = \frac{1}{\bbra{Y_α}_V} \left( \ndt{\vr}_α - \vnab  \cdot (ρ \vb{V}_{\!α} Y_α) \right), \\
\implies \quad \Aboxed{ S_d &= \frac{1}{ρ \norm{\vnab c} \bbra{Y_α}_V} \left[ \ndt{\vr}_α - \vnab  \cdot (ρ \vb{V}_{\!α} Y_α) \right] \bigg|_{F^-} . }
\end{align}
\end{subequations}
...
\begin{subequations}
\begin{align}
(-\bb{K}) &= S_a (\vnab \cdot\uvec{n}) + \uvec{n} \cdot \vnab  \cross (\vb{u}\cross \vb{n}) \Big|_{F^-} \\
&= S_d (\vnab \cdot\uvec{n}) + \left[ (\uvec{n}\otimes\uvec{n}) : \vnab \vb{u} - \vnab  \cdot \vb{u} \right] \Big|_{F^-} \\
&= S_d (\vnab \cdot\uvec{n}) + \left(\uvec{t}_1\otimes\uvec{t}_1 + \uvec{t}_2\otimes\uvec{t}_2\right) : \bb{E} \,\Big|_{F^-} \\
&= S_d \kappa + E
\end{align}
\end{subequations}
...
\begin{subequations}
\begin{align}
\kappa &\equiv \vnab \cdot\uvec{n}, \\
E &\equiv \left(\uvec{t}_1\otimes\uvec{t}_1 + \uvec{t}_2\otimes\uvec{t}_2\right) : \bb{E} \,\Big|_{F^-} = \left[ (\uvec{n}\otimes\uvec{n}) : \vnab \vb{u} - \vnab  \cdot \vb{u}\right] \, \Big|_{F^-}, \\
\bb{E} &= \frac{1}{2}\left(\vnab \vb{u} + (\vnab \vb{u})^T\right)
\end{align}
...
γ_{c^*, α} &\equiv \Big\{ \vb{γ}_{c^*, \s}(s) \, &&| \  \vb{γ}_{c^*, \s}' =  \vnab c / \norm{\vnab c} \big|_{\vb{γ}_{c^*, \s}} \ \text{and} \  \vb{γ}_{c^*, \s}(0) = \vb{\chi}_{c^*}(\sigma) \Big\}
\end{alignat}
\end{subequations}
