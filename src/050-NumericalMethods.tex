...
\end{alignat}
\end{subequations}
for $\a = 1, \dots, N_{\rm{S}}$. The energy $e$ modelled here is the internal energy \emph{of a perfect gas} rather than the total energy specified in \chap{ch:govern-eqns}. The algebraic equations for ideal gas and internal energy (so we ignore the constant chemical potential) for closure are:
\begin{equation}
