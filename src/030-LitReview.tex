...
\cite{yang2015LargeEddySimulationPresent, veynante2002TurbulentCombustionModeling}
...
Needless to say, the cost of simulation must be decreased before useful research may done. A simple way to do this is by modelling simpler physics in DNS than you know is present in the physical system. This may be done .. But it should be noted that benefits to accuracy in DNS are greatly undone when models are used which change the simplify the physical system. To this end, another paradigm is often used for simulations, called \emph{Large Eddy Simulation} (LES) where the same complex physics may be modelled with a coarser resolution. LES is discussed further in the next section. 
% reduce by:
